...
int main(void){
	...
	for(int i = 0; i < 100; i++) { 
        ...
        // !TRUE=FALSE
        {
            std::cerr << "Errore: Bubblesort ha fallito sui numeri!\n";
            return EXIT_FAILURE; 
        }
    }
    ...
    bubble_sort(test_stringhe);
    if(!is_sorted(test_stringhe)){
        std::cerr << "Errore: Bubblesort ha fallito sulle stringhe!\n";
        return EXIT_FAILURE;
    }

    //Se si arriva qui significa che non si è commesso nessun errore 
    std::cout << "Test Bubblesort superato con successo!\n";
    return EXIT_SUCCESS;	
